...
import { Plus, Trash, Edit, Newspaper, CalendarDays, Users, Trophy, LogIn, LogOut, Shield, Settings2, Image as ImageIcon, FileText } from "lucide-react";
...
type Tab = "news" | "events" | "clubs" | "results" | "gallery" | "settings" | "documents";
...
const FileUploadPicker = ({ value, onChange, label = "Dokument (PDF)", accept = "application/pdf" }: { value: string, onChange: (url: string) => void, label?: string, accept?: string }) => {
  return (
    <div className="flex flex-col gap-1 w-full mt-2 mb-2">
      <label className="text-sm font-medium">{label}</label>
      <div className="flex gap-2">
        <input className="w-full border rounded-lg px-3 py-2 bg-gray-50 flex-1" placeholder="URL dokumenta" value={value} onChange={e => onChange(e.target.value)} />
        <Button
          type="button"
          variant="outline"
          className="relative overflow-hidden cursor-pointer"
        >
          ...
          <input 
            ...
            onChange={async (e) => {
               ...
               const formData = new FormData();
               formData.append("files", file);
               toast.loading("Učitavanje dokumenta...", { id: "upload-file" });
               try {
                 const res = await fetch("/api/upload", { method: "POST", body: formData });
                 const data = await res.json();
                 if (data.urls && data.urls.length > 0) {
                     onChange(data.urls[0]);
                     toast.success("Dokument uspješno učitan", { id: "upload-file" });
                 }
               } catch (err) {
                 toast.error("Greška pri učitavanju dokumenta", { id: "upload-file" });
               }
               e.target.value = '';
            }}
          />
...
export default function Admin() {
  ...
  const [newsForm, setNewsForm] = useState({ title: "", excerpt: "", content: "", category: "Obavijest", imageUrl: "", createdAt: "" });

  // Events form state
  const [showEventForm, setShowEventForm] = useState(false);
  const [eventForm, setEventForm] = useState({ title: "", eventDate: "", eventTime: "", location: "", type: "Natjecanje", imageUrl: "", description: "" });

  // Clubs form state
  const [showClubForm, setShowClubForm] = useState(false);
  const [clubForm, setClubForm] = useState({ name: "", city: "", address: "", phone: "", email: "", website: "", imageUrl: "", mapUrl: "", memberCount: "", foundedYear: "", managerName: "" });
  ...
  const [documentForm, setDocumentForm] = useState({ title: "", description: "", category: "Općenito", pdfUrl: "" });
  ...
  const documentsQuery = trpc.documents.list.useQuery(undefined, { enabled: !!user });
  ...
  const handleAddNews = () => {
    ...
    action.mutate({ ...(editingNewsId ? { id: editingNewsId } : {}), ...newsForm } as any, {
      onSuccess: () => {
        newsQuery.refetch();
        setNewsForm({ title: "", excerpt: "", content: "", category: "Obavijest", imageUrl: "", createdAt: "" });
  ...
  const handleAddEvent = () => {
    ...
    action.mutate({ ...(editingEventId ? { id: editingEventId } : {}), ...eventForm } as any, {
      onSuccess: () => {
        eventsQuery.refetch();
        setEventForm({ title: "", eventDate: "", eventTime: "", location: "", type: "Natjecanje", imageUrl: "", description: "" });
  ...
  const handleAddClub = () => {
    ...
    action.mutate({ ...(editingClubId ? { id: editingClubId } : {}), ...clubForm } as any, {
      onSuccess: () => {
        clubsQuery.refetch();
        setClubForm({ name: "", city: "", address: "", phone: "", email: "", website: "", imageUrl: "", mapUrl: "", memberCount: "", foundedYear: "", managerName: "" });
  ...
  const handleAddDocument = () => {
    ...
    const action = editingDocumentId ? updateDocumentMutation : createDocumentMutation;
    action.mutate({ ...(editingDocumentId ? { id: editingDocumentId } : {}), ...documentForm } as any, {
      onSuccess: () => {
        documentsQuery.refetch();
        setDocumentForm({ title: "", description: "", category: "Općenito", pdfUrl: "" });
        setShowDocumentForm(false);
        setEditingDocumentId(null);
        toast.success(editingDocumentId ? "Dokument ažuriran!" : "Dokument kreiran!");
      },
      onError: () => toast.error("Greška pri spremanju dokumenta.")
    });
  };

  const handleDeleteDocument = (id: number) => {
    deleteDocumentMutation.mutate(id, { onSuccess: () => documentsQuery.refetch() });
  };
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: "documents", label: "Dokumenti", icon: <FileText className="w-4 h-4" /> },
      ...
      <section className="py-6 border-b border-border shadow-sm sticky top-16 xl:top-0 z-30 bg-white">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <CardContent className="p-4 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input type="datetime-local" className="w-full border rounded-lg px-3 py-2" value={newsForm.createdAt ? new Date(newsForm.createdAt).toISOString().slice(0, 16) : ""} onChange={e => setNewsForm(p => ({ ...p, createdAt: new Date(e.target.value).toISOString() }))} />
                      </div>
                    </div>
                    <div>
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button className="text-gray-400 hover:text-blue-600" onClick={() => {
                          ...
                          setNewsForm({ title: item.title, excerpt: item.excerpt, content: item.content || "", category: item.category, imageUrl: item.imageUrl || "", createdAt: item.createdAt });
                ...
                <Card className="mb-4">
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1">Vrijeme (npr. 09:00-17:00)</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={eventForm.eventTime} onChange={e => setEventForm(p => ({ ...p, eventTime: e.target.value }))} />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Opis događaja</label>
                      <textarea className="w-full border rounded-lg px-3 py-2" rows={3} value={eventForm.description} onChange={e => setEventForm(p => ({ ...p, description: e.target.value }))} />
                    </div>
                    <div>
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button className="text-gray-400 hover:text-blue-600" onClick={() => {
                          ...
                          setEventForm({ title: item.title, eventDate: item.eventDate, eventTime: item.eventTime || "", location: item.location, type: item.type, imageUrl: item.imageUrl || "", description: item.description || "" });
              ...
              {showClubForm && (
                <Card className="mb-4">
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1">Broj članova (npr. 100+)</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.memberCount} onChange={e => setClubForm(p => ({ ...p, memberCount: e.target.value }))} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1">Godina osnutka (npr. 1950)</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.foundedYear} onChange={e => setClubForm(p => ({ ...p, foundedYear: e.target.value }))} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1">Voditelj kluba</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.managerName} onChange={e => setClubForm(p => ({ ...p, managerName: e.target.value }))} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1">Adresa</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.address} onChange={e => setClubForm(p => ({ ...p, address: e.target.value }))} />
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1">Telefon</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.phone} onChange={e => setClubForm(p => ({ ...p, phone: e.target.value }))} />
                      </div>
                      <div>
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1">Web stranica</label>
                        <input className="w-full border rounded-lg px-3 py-2" value={clubForm.website} onChange={e => setClubForm(p => ({ ...p, website: e.target.value }))} />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Google Maps (Standardni link ili Embed)</label>
                      <input className="w-full border rounded-lg px-3 py-2" placeholder="Npr. https://maps.google.com/..." value={clubForm.mapUrl} onChange={e => setClubForm(p => ({ ...p, mapUrl: e.target.value }))} />
                      <p className="text-xs text-muted-foreground mt-1">
                        Zalijepite i standardni link ("Share") ili Embed kod ("Embed a map" iframe src)
                      </p>
                    </div>
                    <ImageUploadPicker 
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button className="text-gray-400 hover:text-blue-600" onClick={() => {
                          ...
                          setClubForm({ name: item.name, city: item.city, address: item.address || "", phone: item.phone || "", email: item.email || "", website: item.website || "", imageUrl: item.imageUrl || "", mapUrl: item.mapUrl || "", memberCount: item.memberCount || "", foundedYear: item.foundedYear || "", managerName: item.managerName || "" });
              ...
              {showResultForm && (
                <Card className="mb-4">
                  <CardContent className="p-4 space-y-4">
                    ...
                    <FileUploadPicker
                      value={resultForm.pdfUrl}
                      onChange={val => setResultForm(p => ({ ...p, pdfUrl: val }))}
                      label="PDF Dokument sa rezultatima (Opcionalno)"
                    />
                    <div>
                      <label className="block text-sm font-medium mb-1">Detalji i Poredak (Podnaslov/Sadržaj)</label>
                      <div className="bg-white rounded-lg">
          ...
          {activeTab === "documents" && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold">Dokumenti ({documentsQuery.data?.length ?? 0})</h2>
              ...
              {showDocumentForm && (
                <Card className="mb-4 bg-blue-50 border-blue-200">
                  <CardContent className="p-4 space-y-4">
                    <div>
                      <label className="block text-sm font-medium mb-1">Naslov dokumenta</label>
                      <input className="w-full border rounded-lg px-3 py-2 bg-white" placeholder="Npr. Pravilnik o natjecanjima" value={documentForm.title} onChange={e => setDocumentForm(p => ({ ...p, title: e.target.value }))} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Opis (Kratak)</label>
                      <input className="w-full border rounded-lg px-3 py-2 bg-white" placeholder="Npr. Obveze klubova kod natjecanja" value={documentForm.description} onChange={e => setDocumentForm(p => ({ ...p, description: e.target.value }))} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Kategorija</label>
                      <select className="w-full border rounded-lg px-3 py-2 bg-white" value={documentForm.category} onChange={e => setDocumentForm(p => ({...p, category: e.target.value}))}>
                        <option value="Općenito">Općenito</option>
                        <option value="Pravni dokumenti">Pravni dokumenti</option>
                        <option value="Pravila">Pravila</option>
                    ...
                    <FileUploadPicker
                      ...
                      onChange={val => setDocumentForm(p => ({ ...p, pdfUrl: val }))}
                      label="PDF Dokument (Obavezno)"
                    />
                    
                    <div className="flex gap-2">
                      <Button onClick={handleAddDocument} disabled={createDocumentMutation.isLoading}>Spremi</Button>
                      <Button variant="outline" onClick={() => setShowDocumentForm(false)}>Odustani</Button>
                    </div>
                  </CardContent>
                </Card>
              )}

              {documentsQuery.isLoading && <p className="text-muted-foreground">Učitavanje...</p>}
              {documentsQuery.data?.map((item: any) => (
                <Card key={item.id} className="mb-3">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1 max-w-lg truncate">{item.description}</p>
                      <div className="flex gap-3 mt-2 text-xs">
                        <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-medium">{item.category}</span>
                        {item.pdfUrl ? (
                          <a href={item.pdfUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline inline-flex items-center">Pregledaj PDF →</a>
                        ) : (
                          <span className="text-red-500">Nema dodanog PDF-a</span>
                        )}
                      </div>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button className="text-gray-400 hover:text-blue-600 p-2" onClick={() => {
                          setEditingDocumentId(item.id);
                          setDocumentForm({ title: item.title, description: item.description || "", category: item.category || "Općenito", pdfUrl: item.pdfUrl || "" });
                          setShowDocumentForm(true);
                      }}><Edit className="w-4 h-4" /></button>
                      <button className="text-gray-400 hover:text-red-600 p-2" onClick={() => {
                          if(confirm("Jeste li sigurni da želite obrisati dokument?")) {
              ...
              {documentsQuery.data?.length === 0 && <p className="text-muted-foreground text-center py-8">Nema dodanih dokumenata. Dodajte prvi!</p>}
            </div>
          )}
