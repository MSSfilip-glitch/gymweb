...
- **WAL Checkpointing:** A cron job (`/var/www/gymWeb/checkpoint.sh`) runs every 5 minutes to flush the SQLite WAL to the main database file. This protects against data loss from unexpected reboots.
- **Daily Backups:** A cron job (`/var/www/gymWeb/backup.sh`) runs at 3:00 AM and saves a clean snapshot to `/var/backups/gymweb/`. The last 7 days are kept.
- **To restore a backup:**
  ```bash
  pm2 stop gszz-api
  cp /var/backups/gymweb/gszz_YYYYMMDD_HHMMSS.db /var/www/gymWeb/gszz.db
  rm -f /var/www/gymWeb/gszz.db-wal /var/www/gymWeb/gszz.db-shm
  pm2 start gszz-api
  ```
...
To enter from terminal:
ssh root@188.245.99.119

To refresh the site after changes on the server:
cd /var/www/gymWeb
git pull
npm run build
pm2 restart gszz-api
