...
import { news, events, clubs, results, users, settings, galleryAlbums, galleryAlbumImages, documents } from "./schema.js";
...
export const appRouter = router({
    ...
    clubs: router({
        ...
        create: protectedProcedure
            .input(z.object({
                ...
                foundedYear: z.string().max(100).optional(),
    ...
    documents: router({
        ...
        delete: protectedProcedure.input(z.number()).mutation(async ({ input }) => {
            db.delete(documents).where(eq(documents.id, input)).run();
            return { success: true };
        }),
    }),
